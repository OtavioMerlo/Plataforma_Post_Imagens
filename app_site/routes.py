...
from app_site import app, database, bcrypt
from flask_login import login_required, login_user, logout_user, current_user
from app_site.forms import FormLogin, FormCriarConta, FormFoto
...
def criar_conta():
    ...
    if form_criarconta.validate_on_submit():
        ...
        usuario = Usuario(
            username=form_criarconta.nome.data,
            password=senha,
...
def perfil(id_usuario):
    if int(id_usuario) == int(current_user.id):
        ...
        if form_Foto.validate_on_submit():
            ...
            database.session.commit()
        return render_template("perfil.html", form=form_Foto, usuario=current_user, dono=True)
    else:
...
@app.route("/view-foto/<id_foto>")
@login_required
def view_foto(id_foto):
    foto=Foto.query.get(id_foto)
    return render_template("view_foto.html", imagem=foto)